...
def test_list_endpoints(test_repo_path, test_config):
    ...
    try:
        ...
        assert (
            len(endpoints) == 7
        )  # tool1, resource1, resource1_detail, prompt1, tool2, prompt2, disabled_tool (disabled_tool filtered out)
        ...
        ), "Disabled endpoint should be filtered out"
    finally:
        os.chdir(original_dir)
...
def test_list_endpoints_from_subfolder(test_repo_path, test_config):
    ...
    try:
        ...
        assert (
            len(endpoints) == 7
        )  # tool1, resource1, resource1_detail, prompt1, tool2, prompt2 (disabled_tool filtered out)
