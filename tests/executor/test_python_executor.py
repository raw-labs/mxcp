...
""")
        ...
        result = await executor.execute("python/test_module.py:multiply", {"a": 4, "b": 5}, mock_context)
...
class TestPythonLifecycleHooks:
    """Test Python lifecycle hook functionality."""
    
    def test_hook_discovery(self, temp_repo_dir, mock_context):
        """Test discovery of lifecycle hooks."""
        executor = PythonExecutor(repo_root=temp_repo_dir)
        
        # Create Python file with hooks
        python_file = temp_repo_dir / "python" / "hooks.py"
        python_file.write_text("""
def init_hook():
...
""")
        ...
        # Should discover hooks
        executor._discover_hooks()
        
        # Check that hooks were discovered
        assert len(executor._init_hooks) >= 0
        assert len(executor._shutdown_hooks) >= 0
    
    def test_init_hooks_execution(self, temp_repo_dir, mock_context):
        """Test execution of initialization hooks."""
        executor = PythonExecutor(repo_root=temp_repo_dir)
        
        # Create Python file with init hook
        python_file = temp_repo_dir / "python" / "test_hooks.py"
        python_file.write_text("""
initialized = False
...
""")
        ...
        # Manually add hook for testing
        module = executor.loader.load_python_module(python_file)
        init_func = executor.loader.get_function(module, "init_test")
        executor._init_hooks = [init_func]
        
        # Run init hooks
        executor._run_init_hooks()
        
        # Check that hook was executed
        get_status = executor.loader.get_function(module, "get_status")
        status = get_status()
        assert status["initialized"] is True
    
    def test_shutdown_hooks_execution(self, temp_repo_dir, mock_context):
        """Test execution of shutdown hooks."""
        executor = PythonExecutor(repo_root=temp_repo_dir)
        
        # Create Python file with shutdown hook
        python_file = temp_repo_dir / "python" / "shutdown_hooks.py"
        python_file.write_text("""
...
def get_shutdown_status():
    return {"shutdown_called": shutdown_called}
""")
        ...
        # Manually add hook for testing
        module = executor.loader.load_python_module(python_file)
        shutdown_func = executor.loader.get_function(module, "shutdown_test")
        executor._shutdown_hooks = [shutdown_func]
        
        # Run shutdown hooks
        executor._run_shutdown_hooks()
        
        # Check that hook was executed
        get_status = executor.loader.get_function(module, "get_shutdown_status")
        status = get_status()
        assert status["shutdown_called"] is True
