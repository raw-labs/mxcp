...
class TestDuckDBSQLExecution:
    ...
    async def test_execute_parameterized_query(self, duckdb_executor, mock_context):
        ...
        result = await duckdb_executor.execute(
            "SELECT $num as number, $text as message", 
            {"num": 42, "text": "hello"},
            mock_context
        )
...
class TestDuckDBErrorHandling:
    ...
    async def test_parameter_missing_error(self, duckdb_executor, mock_context):
        ...
        with pytest.raises(Exception) as exc_info:
            await duckdb_executor.execute("SELECT * FROM table WHERE id = $missing_param", {}, mock_context)
    ...
    async def test_constraint_violation_error(self, duckdb_executor, mock_context):
        ...
        # Insert first record
        await duckdb_executor.execute("INSERT INTO unique_test VALUES (1, 'test')", {}, mock_context)
        
        # Try to insert duplicate - should raise constraint violation
        with pytest.raises(Exception) as exc_info:
            await duckdb_executor.execute("INSERT INTO unique_test VALUES (2, 'test')", {}, mock_context)
    ...
    async def test_type_mismatch_error(self, duckdb_executor, mock_context):
        ...
        with pytest.raises(Exception) as exc_info:
            await duckdb_executor.execute("INSERT INTO type_test VALUES (1, 'not_a_number')", {}, mock_context)
        
        # Should contain information about the type mismatch
        assert "type" in str(exc_info.value).lower() or "cast" in str(exc_info.value).lower() or "conversion" in str(exc_info.value).lower()
...
class TestDuckDBSessionManagement:
    ...
    def test_session_persistence(self, duckdb_executor, mock_context):
        ...
        # Create table
        asyncio.run(duckdb_executor.execute("CREATE TABLE persist_test (id INTEGER)", {}, mock_context))
        ...
        result = asyncio.run(duckdb_executor.execute("SELECT * FROM persist_test", {}, mock_context))
        assert result == []
    ...
    def test_memory_database_default(self, duckdb_executor, mock_context):
        ...
        asyncio.run(duckdb_executor.execute("""
...
class TestDuckDBExtensionsAndPlugins:
    ...
    def test_json_extension_available(self, duckdb_executor, mock_context):
        ...
        result = asyncio.run(duckdb_executor.execute("""
