...
def test_validate_valid_endpoint(validation_repo_path, site_config, user_config):
    """Test validation of a valid endpoint."""
    original_dir = os.getcwd()
    os.chdir(validation_repo_path)
    try:
        endpoint_path = "endpoints/valid_endpoint.yml"
        result = validate_endpoint(endpoint_path, site_config, user_config, None)
...
def test_validate_invalid_type(validation_repo_path, site_config, user_config):
...
def test_validate_missing_param(validation_repo_path, site_config, user_config):
...
def test_validate_all_endpoints(validation_repo_path, site_config, user_config):
    """Test validation of all endpoints in the repository."""
    original_dir = os.getcwd()
    os.chdir(validation_repo_path)
    try:
        result = validate_all_endpoints(site_config, user_config, None)
        assert result["status"] == "ok"
