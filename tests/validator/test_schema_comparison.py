...
class TestSchemaComparison:
    ...
    def test_compare_with_common_types(self):
        ...
        # Load both schemas
        validation_schema_path = Path(__file__).parent.parent.parent / "src" / "mxcp" / "validator" / "schemas" / "validation-schema-1.json"
        common_types_path = Path(__file__).parent.parent.parent / "src" / "mxcp" / "endpoints" / "schemas" / "common-types-schema-1.json"
        ...
        assert set(validation_types) == set(common_types), f"Type enums differ: {validation_types} vs {common_types}"
        ...
        assert set(validation_formats) == set(common_formats), f"Format enums differ: {validation_formats} vs {common_formats}"
    ...
    def test_schema_alignment_summary(self):
        ...
        validation_types = validation_schema["definitions"]["typeEnum"]["enum"]
        common_types = common_types_schema["definitions"]["typeDefinition"]["properties"]["type"]["enum"]
        assert validation_types == common_types
        
        # Verify format enums match exactly
        validation_formats = validation_schema["definitions"]["formatEnum"]["enum"]
        common_formats = common_types_schema["definitions"]["typeDefinition"]["properties"]["format"]["enum"]
