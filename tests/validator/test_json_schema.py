...
class TestJSONSchemaValidation:
    ...
    def test_json_schema_exists(self):
        ...
        schema_path = Path(__file__).parent.parent.parent / "src" / "mxcp" / "validator" / "schemas" / "validation-schema-1.json"
    ...
    def test_validate_with_json_schema(self):
        ...
        valid_schema = {
            "input": {
                "parameters": [
                    {"name": "x", "type": "string", "minLength": 1}
                ]
            },
            "output": {
                "type": "object",
        ...
        with pytest.raises(ValueError, match="is not one of"):
            validate_schema_structure({
                "input": {
                    "parameters": [
                        {"name": "x", "type": "invalid_type"}  # Not in enum
                    ]
                }
            })
        ...
        with pytest.raises(ValueError, match="is not one of"):
            validate_schema_structure({
                "input": {
                    "parameters": [
                        {"name": "x", "type": "string", "format": "invalid_format"}
                    ]
                }
            })
