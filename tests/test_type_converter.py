...
def test_string_formats():
    ...
    # Email format
    assert (
        TypeConverter.convert_value("test@example.com", {"type": "string", "format": "email"})
        == "test@example.com"
    )
    ...
    # URI format
    assert (
        TypeConverter.convert_value("https://example.com", {"type": "string", "format": "uri"})
        == "https://example.com"
    )
    with pytest.raises(ValueError):
    ...
    # Duration format
    assert (
        TypeConverter.convert_value("P1DT2H", {"type": "string", "format": "duration"}) == "P1DT2H"
    )
    with pytest.raises(ValueError):
...
def test_numeric_constraints():
    ...
    with pytest.raises(ValueError):
        TypeConverter.convert_value(
            "-1", {**param_def, "exclusiveMinimum": 0}
        )  # Not greater than exclusiveMinimum
    with pytest.raises(ValueError):
        TypeConverter.convert_value(
            "101", {**param_def, "exclusiveMaximum": 100}
        )  # Not less than exclusiveMaximum
...
def test_string_constraints():
    ...
    param_def = {"type": "string", "minLength": 2, "maxLength": 5}
...
def test_string_format():
    ...
    # Email format
    param_def = {"type": "string", "format": "email"}
    ...
    # URI format
    param_def = {"type": "string", "format": "uri"}
    ...
    # Date format
    param_def = {"type": "string", "format": "date"}
    ...
    # Time format
    param_def = {"type": "string", "format": "time"}
    ...
    # Date-time format
    param_def = {"type": "string", "format": "date-time"}
...
def test_object_constraints():
    ...
    with pytest.raises(ValueError):
        TypeConverter.convert_value(
            {"name": "John", "age": 30, "email": "invalid-email"}, param_def
        )
