...
from mxcp.config.references import ONEPASSWORD_URL_PATTERN, interpolate_all, resolve_onepassword_url
...
class TestOnePasswordIntegration:
    ...
    def test_onepassword_url_parsing(self):
        ...
        op_config = {"enabled": True, "token_env": "OP_SERVICE_ACCOUNT_TOKEN"}
        ...
        with patch.dict(os.environ, {"OP_SERVICE_ACCOUNT_TOKEN": "test-token"}):
            with patch("onepassword.Client") as mock_client_class:
                ...
                mock_client.secrets.resolve.assert_called_once_with(
                    "op://test-vault/test-item/password"
                )
    ...
    def test_onepassword_url_otp_attribute(self):
        ...
        with patch.dict(os.environ, {"OP_SERVICE_ACCOUNT_TOKEN": "test-token"}):
            with patch("onepassword.Client") as mock_client_class:
                ...
                # Test OTP resolution
                result = resolve_onepassword_url(
                    "op://test-vault/test-item/otp?attribute=otp", op_config
                )

                assert result == "123456"
                mock_client.secrets.resolve.assert_called_once_with(
                    "op://test-vault/test-item/otp?attribute=totp"
                )
    ...
    def test_onepassword_missing_token(self):
        ...
        op_config = {"enabled": True, "token_env": "MISSING_TOKEN"}
    ...
    def test_onepassword_interpolation_in_config(self):
        ...
        config = {
            ...
            "api": {"key": "op://vault/api-keys/production"},
        }
