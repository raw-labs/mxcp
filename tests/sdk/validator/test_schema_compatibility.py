    ...
    def test_endpoint_schema_structure(self):
        ...
        tool_endpoint = {
            ...
            "tool": {
                ...
                "return": {"type": "number", "description": "Sum of the numbers"},
            },
        }

        # Extract validation schema from endpoint
        validation_schema = {
            "input": {"parameters": tool_endpoint["tool"]["parameters"]},
    ...
    def test_resource_endpoint_validation(self):
        ...
        resource_endpoint = {
            ...
            "resource": {
                ...
                "parameters": [
                    {"name": "user_id", "type": "integer", "minimum": 1, "description": "User ID"}
                ],
                "return": {
                    "type": "object",
                    "properties": {
        ...
        validation_schema = {
            "input": {"parameters": resource_endpoint["resource"]["parameters"]},
    ...
    def test_prompt_endpoint_validation(self):
        ...
        prompt_endpoint = {
            ...
            "prompt": {
                ...
                "messages": [
                    {"role": "user", "prompt": "Analyze this {{language}} code:\n\n{{code}}"}
                ],
            },
        }

        # Extract validation schema (prompts typically only have input)
        validation_schema = {"input": {"parameters": prompt_endpoint["prompt"]["parameters"]}}

        validator = TypeValidator.from_dict(validation_schema)

        # Valid input
        result = validator.validate_input({"code": "def hello(): pass", "language": "python"})
        assert result["language"] == "python"

        # Invalid language
        with pytest.raises(ValidationError, match="Must be one of"):
            validator.validate_input(
                {"code": "function hello() {}", "language": "typescript"}  # Not in enum
            )
