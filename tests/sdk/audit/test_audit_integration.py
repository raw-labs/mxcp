...
async def test_complete_audit_workflow():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        # Get all auth events
        auth_events = [
            r async for r in logger.query_records(schema_name="auth_operations", limit=10)
        ]
        ...
        # Get all API events
        api_events = [r async for r in logger.query_records(schema_name="api_operations", limit=10)]
...
async def test_high_volume_logging():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        error_events = [
            r
            async for r in logger.query_records(
                schema_name="volume_test",
        ...
        # Test user filtering
        user_0_events = [
            r async for r in logger.query_records(schema_name="volume_test", user_ids=["user_0"])
        ]
