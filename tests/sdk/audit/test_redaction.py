...
def test_partial_redaction():
    ...
    # Custom options
    assert apply_redaction("1234567890", RedactionStrategy.PARTIAL, 
                          {"show_first": 0, "show_last": 4}) == "***7890"
    assert apply_redaction("1234567890", RedactionStrategy.PARTIAL,
                          {"show_first": 3, "show_last": 0}) == "123***"
...
def test_truncate_redaction():
    ...
    # Custom length
    assert apply_redaction("verylongstring", RedactionStrategy.TRUNCATE,
                          {"length": 5}) == "veryl..."
...
def test_redaction_options():
    ...
    result = apply_redaction("1234567890", RedactionStrategy.PARTIAL, {
        "show_first": 4,
        "show_last": 2,
        "min_length": 6
    })
    ...
    result = apply_redaction("user@example.com", RedactionStrategy.EMAIL, {
        "preserve_domain": False
    })
    assert result == "[REDACTED]"
    
    # Truncate options
    result = apply_redaction("verylongstring", RedactionStrategy.TRUNCATE, {
        "length": 8,
        "suffix": "!!!"
    })
