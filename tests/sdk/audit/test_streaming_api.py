...
async def test_streaming_memory_efficiency():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        # Create test schema
        schema = AuditSchema(
            schema_name="memory_test", version=1, description="Test schema for memory efficiency"
        )
        await logger.create_schema(schema)
...
async def test_streaming_with_filters():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        # Test user filter
        user_0_count = 0
        async for record in logger.query_records(schema_name="filter_test", user_ids=["user_0"]):
        ...
        async for record in logger.query_records(
            schema_name="filter_test", operation_status=["error"]
        ):
        ...
        deny_count = 0
        async for record in logger.query_records(
            schema_name="filter_test", policy_decisions=["deny"]
        ):
        ...
        combined_count = 0
        async for record in logger.query_records(
            schema_name="filter_test", user_ids=["user_0"], operation_status=["error"]
        ):
            assert record.user_id == "user_0"
            assert record.operation_status == "error"
            combined_count += 1
...
async def test_concurrent_streaming():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        async def count_user_records(user_id: str) -> int:
            count = 0
            async for record in logger.query_records(
                schema_name="concurrent_test", user_ids=[user_id]
            ):
