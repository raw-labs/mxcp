...
Plugins must contain a class named `MXCPPlugin`:
```python
...
```
drift/
├── snapshot-default.json   # Default profile
├── snapshot-staging.json   # Staging profile
└── snapshot-prod.json      # Production profile
```
...
- Example tool with SQL
- Example tool with Python
...
- [Endpoints](endpoints) - Endpoint definition format
