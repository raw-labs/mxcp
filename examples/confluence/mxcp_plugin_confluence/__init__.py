...
class MXCPPlugin(MXCPBasePlugin):
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        if not all([self.url, self.username, self.password]):
            raise ValueError("Confluence plugin requires url, username, and password in configuration")
    ...
    @udf
    def cql_query(self, query: str, space_key: Optional[str] = None, max_results: Optional[int] = 50) -> str:
        ...
        """
        logger.info(f"Executing CQL query: {query} in space={space_key} with max_results={max_results}")
        ...
        transformed_results = [
            {
                ...
                "labels": [label['name'] for label in page['content']['metadata']['labels']['results']]
            }
    ...
    def get_children(self, page_id: str) -> str:
        ...
        children = self.confluence.get_child_pages(
            page_id=page_id,
            expand='version,space'
        )
    ...
    def list_spaces(self) -> str:
        ...
        spaces = self.confluence.get_all_spaces(
            expand='description,metadata.labels'
        )
