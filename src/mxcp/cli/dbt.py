...
def dbt_config(profile: str, dry_run: bool, force: bool, embed_secrets: bool, debug: bool):
    ...
    if embed_secrets:
        click.echo(f"   • Mode: {click.style('Embed secrets (⚠️  not recommended for production)', fg='red')}")
    else:
        click.echo(f"   • Mode: {click.style('Environment variables (secure)', fg='green')}")
    
    if dry_run:
        click.echo(f"   • {click.style('DRY RUN MODE', fg='magenta', bold=True)} - No files will be modified")
...
def dbt_wrapper(ctx, debug: bool):
    ...
    if secret_count > 0:
        click.echo(f"   • Secrets: {click.style(f'{secret_count} environment variables injected', fg='green')}")
    ...
    try:
        ...
        click.echo(f"\n" + "-" * 60)
        click.echo(f"{click.style('✅ dbt command completed successfully!', fg='green', bold=True)}\n")
    except subprocess.CalledProcessError as e:
        click.echo(f"\n" + "-" * 60)
        click.echo(f"{click.style('❌ dbt command failed with exit code:', fg='red', bold=True)} {e.returncode}\n")
