        ...
        for file_path, endpoint, error_msg in endpoints:
            ...
            try:
                ...
                test_results = await run_tests_with_session(
                    kind, name, user_config, site_config, session, profile
                )
...
async def run_tests(
    ...
    try:
        return await run_tests_with_session(
            endpoint_type, name, user_config, site_config, session, profile
        )
    finally:
        session.close()
...
async def run_tests_with_session(
    ...
    try:
        ...
        if not tests:
            ...
            return {"status": "ok", "tests_run": 0, "message": "No tests defined"}
        ...
        for test_def in tests:
            ...
            try:
                ...
                result = await execute_endpoint(
                    endpoint_type, name, params, user_config, site_config, session, profile
                )
...
def get_endpoint_source_code(
    endpoint_def: dict, endpoint_type: str, endpoint_file_path: Path, repo_root: Path
) -> str:
