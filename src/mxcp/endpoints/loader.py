                    ...
                    with open(f) as file:
                        ...
                        if endpoint_type in data:
                            ...
                            if endpoint_type == "tool" and "name" in data["tool"]:
                                logger.debug(
                                    f"Tool name: {data['tool']['name']}, looking for: {name}"
                                )

                            if endpoint_type == "resource" and "uri" in data["resource"]:
                                logger.debug(
                                    f"Resource URI: {data['resource']['uri']}, looking for: {name}"
                                )

                            if endpoint_type == "prompt" and "name" in data["prompt"]:
                                logger.debug(
                                    f"Prompt name: {data['prompt']['name']}, looking for: {name}"
                                )

                        # Check if this is the endpoint we're looking for
                        if endpoint_type in data and (
                            (endpoint_type == "tool" and data["tool"]["name"] == name)
                            or (endpoint_type == "resource" and data["resource"]["uri"] == name)
                            or (endpoint_type == "prompt" and data["prompt"]["name"] == name)
                        ):
                            ...
                            schema_path = (
                                Path(__file__).parent / "schemas" / "endpoint-schema-1.0.0.json"
                            )
