...
class SalesforceOAuthHandler(ExternalOAuthHandler):
    ...
    async def exchange_code(self, code: str, state: str) -> ExternalUserInfo:
        ...
        logger.info(f"Salesforce OAuth token exchange: code={code[:10]}..., redirect_uri={full_callback_url}")
    ...
    async def get_user_context(self, token: str) -> UserContext:
        ...
        try:
            ...
            return UserContext(
                ...
                username=user_profile.get("username", f"user_{user_profile.get('user_id', 'unknown')}"),
    ...
    async def _fetch_user_profile(self, token: str) -> dict[str, Any]:
        ...
        if resp.status_code != 200:
            ...
            try:
                ...
                logger.error(f"Salesforce UserInfo API error {resp.status_code}: Unable to read response body")
            raise ValueError(f"Salesforce API error: {resp.status_code} - {error_body}")
        
        user_data = resp.json()
        logger.info(f"Successfully fetched Salesforce user profile for user_id: {user_data.get('user_id', 'unknown')}")
        return user_data 