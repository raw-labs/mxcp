...
class KeycloakOAuthHandler(ExternalOAuthHandler):
    ...
    async def on_callback(self, request: Request, provider: "GeneralOAuthAuthorizationServer") -> Response:
        ...
        if error:
            ...
            return HTMLResponse(
                content=f"<h1>Authentication Failed</h1><p>{error_description}</p>",
                status_code=400
            )
