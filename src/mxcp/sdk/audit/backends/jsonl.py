...
from ..types import AuditRecord, AuditSchema, IntegrityResult, EvidenceLevel, Status, PolicyDecision
...
class JSONLAuditWriter(BaseAuditWriter):
    ...
    def _dict_to_schema(self, d: Dict[str, Any]) -> AuditSchema:
        ...
        from ..types import FieldDefinition, FieldRedaction
        ...
        if 'field_redactions' in d:
            ...
            for r in d['field_redactions']:
                ...
                from ..types import RedactionStrategy
                strategy = RedactionStrategy(r.get('strategy', 'full'))
                redactions.append(FieldRedaction(
    ...
    async def get_schema(
        ...
        if version is not None:
            ...
            matching_schemas = [
                s for s in schemas 
                if s.schema_name == schema_name and s.active
            ]
    ...
    def _start_writer_thread(self):
        ...
        for attempt in range(max_attempts):
            try:
                ...
                logger.debug(f"Background writer thread started for {self.log_path} (attempt {attempt + 1})")
                ...
                if self._writer_thread.is_alive():
                    ...
                    logger.error(f"Thread startup FAILED - thread died immediately (attempt {attempt + 1})")
                    
            except Exception as e:
                logger.error(f"FAILED to start background writer thread (attempt {attempt + 1}): {e}")
        ...
        # All attempts failed
        raise RuntimeError(f"Failed to start background writer thread after {max_attempts} attempts")
        ...
        while True:
            ...
            try:
                ...
                for row in result:
                    ...
                    record_fields = {k: v for k, v in row_dict.items() 
    ...
    async def apply_retention_policies(self) -> Dict[str, int]:
        ...
        try:
            ...
            logger.info(f"Retention applied: {retained_count} records retained, "
