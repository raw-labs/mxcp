...
def _load_plugin(
    module_path: str, config: Dict[str, str], user_context: Optional["UserContext"] = None
) -> MXCPBasePlugin:
    ...
    try:
        ...
        if supports_user_context:
            ...
            logger.debug(
                f"Plugin {module_path} does not support user context (backward compatibility mode)"
            )
    ...
    for plugin_def in site_config["plugin"]:
        ...
        if config_name is not None:
            if config_name not in plugin_config:
                raise ValueError(
                    f"Plugin configuration '{config_name}' not found in user config for profile {profile}"
                )
        ...
        if user_context:
            logger.info(
                f"Loaded plugin {name} from {module} with user context for {user_context.username}"
            )
        else:
