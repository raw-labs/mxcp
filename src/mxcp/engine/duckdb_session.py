...
def execute_query_to_dict(
    conn: duckdb.DuckDBPyConnection, query: str, params: Dict[str, Any] = None
) -> List[Dict[str, Any]]:
    """
...
class DuckDBSession:
    ...
    def _connect(self):
        ...
        self.plugins = load_plugins(
            self.site_config, self.user_config, project, profile_name, self.conn, user_context
        )
    ...
    def _create_user_token_udfs(self):
        ...
        if self.conn:
            self.conn.create_function(
                "get_user_external_token", get_user_external_token, [], "VARCHAR"
            )
