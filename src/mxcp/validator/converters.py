...
import numpy as np
import pandas as pd
from .types import TypeSchema, ParameterSchema
        ...
        if param_type == "string":
            ...
            if param_format == "date-time":
                if not isinstance(value, str):
                    raise ValidationError(f"Expected string for date-time format, got {type(value).__name__}")
                return datetime.fromisoformat(value.replace("Z", "+00:00"))
            elif param_format == "date":
                if not isinstance(value, str):
                    raise ValidationError(f"Expected string for date format, got {type(value).__name__}")
                return datetime.strptime(value, "%Y-%m-%d").date()
            elif param_format == "time":
                if not isinstance(value, str):
                    raise ValidationError(f"Expected string for time format, got {type(value).__name__}")
            ...
            if schema.min_length is not None and len(value) < schema.min_length:
                raise ValidationError(f"String must be at least {schema.min_length} characters long")
    ...
    def validate_output(value: Any, schema: TypeSchema) -> None:
        ...
        if isinstance(value, pd.DataFrame):
            ...
            if schema.items and schema.items.type != "object":
                raise ValidationError(f"DataFrame rows must be objects, but schema expects array of {schema.items.type}")
