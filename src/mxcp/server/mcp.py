...
class RAWMCP:
    ...
    def __init__(
        ...
        self.oauth_handler = create_oauth_handler(
            auth_config, host=host, port=port, user_config=user_config
        )
        ...
        logger.info(
            f"FastMCP settings after initialization: host={self.mcp.settings.host}, port={self.mcp.settings.port}"
        )

        # Initialize authentication middleware
        self.auth_middleware = AuthenticationMiddleware(self.oauth_handler, self.oauth_server)
        logger.info(
            f"Authentication middleware initialized with oauth_handler: {self.oauth_handler}, oauth_server: {self.oauth_server}"
        )
        ...
        self.skipped_endpoints = [
            ...
            for path, _, error in discovered
        ...
        if audit_config.get("enabled", False):
            self.audit_logger = AuditLogger(log_path=Path(audit_config["path"]), enabled=True)
        else:
